...
└── lessons/
    ...
    │       ├── food-system.png
    │       └── hidden-costs.png
    └── 2026-09-10/                  # 由 build.py 產生，自帶樣式與引擎
...
兩個單元的投影片引擎目前並不共用：`2026-09-04` 使用根目錄的 `styles.css` 與 `app.js`，`2026-09-10` 是 `build.py` 產出的自帶樣式版本。共用的是課程首頁、色票與字型。日後若要統一，再把新單元收斂到其中一種寫法。
...
`2026-09-10` 單元：方向鍵換頁，`N` 開關備註，`F` 全螢幕，底部控制列另有目錄、教師備課與返回課程首頁。遊戲進行時務必關閉備註，備註內含臥底題目。
